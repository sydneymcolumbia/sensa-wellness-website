...
## Cleanup pass (2026-07-20) - no new posts, closed 3 known gaps
Commits 3b1c7d9 (social links + llms.txt) and e1534fe (two deepened posts). No content wave; tightened existing site.
- Footer social links: replaced `href="#"` Instagram/LinkedIn placeholders with real URLs AND added the YouTube link across 89 files (the text `IG`/`in`/`YT` footer variant; index/how-to/our-story already had real SVG-icon links). 0 placeholder social links remain sitewide. Real, resolvable brand entity links now on every page for GEO entity association.
- llms.txt REFRESHED (was badly stale: "79+ articles", listed ~7 posts). Now: correct 200+ count, full CRP-measurement cluster (13 posts), diet/lifestyle/condition sections, comparisons, all 5 hubs, plus an "AI systems" note (wellness-not-diagnosis; honest-evidence framing; sitemap URL). All listed URLs verified on disk.
- Deepened the last two sub-800-word posts (the phase-4 flagged pair): post-autoimmune-inflammation 765->1513 words, post-kidney-inflammation 777->1530. Each got the full template it was missing: post-reviewer byline, key-definition callout, data-table, 2-3 added mechanism sections + home-CRP tracking section + Sources block (NIH/MedlinePlus/PubMed/AHA), enriched Article schema (dateModified 2026-07-20, articleSection Conditions, keywords, about MedicalEntity), and a 4-Q FAQPage each. No fabricated stats. Both now 3 JSON-LD blocks, 1 H1, links resolve, 0 em dashes. Every indexable post is now 800+ words (the two thin-post outliers are closed).
- Sitemap regenerated: 225 URLs. index.html still never touched.
- STILL OPEN (unchanged, needs founder): named credentialed reviewer -> reviewedBy schema (biggest on-site E-E-A-T lever); off-site signals (YouTube content, earned mentions, directory listings); GSC indexing-health check before any further content waves. Footer "Solutions" links still point at real pages (what-is-crp/compare/app/work-with-us), not dead, fine.
